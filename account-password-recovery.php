...
if(isset($_POST['submit'])){
    $email = $_POST['email'];
    $sql = $con->query("SELECT * FROM custmors WHERE email = '$email' ");
    $info = $sql->fetch_assoc();

    $str = "0123456789qwertzuioplkjhgfdsayxcvbnm";
    $str = str_shuffle($str);
    $str = substr($str, 0, 10);
    $url = "https://localhost/template-3/account-new-password.php?token=$str&email=$email";
    $con->query("UPDATE custmors SET token='$str', expire = DATE_ADD(NOW(), INTERVAL 5 MINUTE) WHERE email='$email'");
    if($info['email'] == $email) {
      require 'PHPMailer/PHPMailerAutoload.php';
          ...
          if ($mail->send())
          {
              $msg = "Please chect your email to reset your password!";
          }
          else{
    ...
    <div class="container padding-bottom-3x mb-2">
      <div class="row justify-content-center">
        <div class="col-lg-8 col-md-10">
          <h4 class="text-success"><?php if (isset($msg)){echo $msg;} ?></h4>
          ...
          <ol class="list-unstyled">
            ...
            <li><span class="text-primary text-medium">2. </span>We'll send an email with a link.</li>
            <li><span class="text-primary text-medium">3. </span>Use the link to reset your password on our secure website.</li>
          </ol>
          <form class="card mt-4"  action="account-password-recovery.php" method="post">
            <div class="card-body">
              <div class="form-group">
                <label for="email-for-pass">Enter your email address</label>
                <input class="form-control" type="text" name="email" id="email-for-pass" required><small class="form-text text-muted">Type in the email address you used when you registered with Build Easy. Then we'll email a link to this address.</small>
              </div>
            </div>
            <div class="card-footer">
              <button class="btn btn-primary" name="submit" type="submit">Get New Password</button>
            </div>
          </form>
        </div>
      </div>
