...
if(isset($_POST['submit'])){
	$email = $_POST['email'];
	$sql = $con->query("SELECT * FROM vendors WHERE email = '$email' ");
	$info = $sql->fetch_assoc();

	$str = "0123456789qwertzuioplkjhgfdsayxcvbnm";
	$str = str_shuffle($str);
	$str = substr($str, 0, 10);
	$url = "https://localhost/build/dashboard/resetpass.php?token=$str&email=$email";
	$con->query("UPDATE vendors SET token='$str', expire = DATE_ADD(NOW(), INTERVAL 5 MINUTE) WHERE email='$email'");
	if($info['email'] == $email) {
